...
import com.dkd.manager.service.INodeService;
...
public class NodeController extends BaseController
{
     ...
     * 查询点位管理列表
     */
    @PreAuthorize("@ss.hasPermi('manager:node:list')")
    @GetMapping("/list")
     ...
     * 导出点位管理列表
     */
    @PreAuthorize("@ss.hasPermi('manager:node:export')")
     ...
     * 获取点位管理详细信息
     */
    @PreAuthorize("@ss.hasPermi('manager:node:query')")
    @GetMapping(value = "/{id}")
     ...
     * 新增点位管理
     */
    @PreAuthorize("@ss.hasPermi('manager:node:add')")
     ...
     * 修改点位管理
     */
    @PreAuthorize("@ss.hasPermi('manager:node:edit')")
     ...
     * 删除点位管理
     */
    @PreAuthorize("@ss.hasPermi('manager:node:remove')")
